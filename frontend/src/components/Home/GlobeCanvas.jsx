...
export default function GlobeCanvas({ onSettle }) {
  ...
  const START_COALESCE = isMobile ? 900 : 2000;
  const END_COALESCE = isMobile ? 1500 : 3800;
  const START_DRIFT = isMobile ? 400 : 3800;
  const END_DRIFT = isMobile ? 1400 : 5000;
  ...
  useEffect(() => {
    ...
    // Globe parameters
    const GLOBE_RADIUS = isMobile ? 150 : 210;
    const CAMERA_DIST = isMobile ? 640 : 760;
    
    // Generate particles
    const particles = [];
    const rows = isMobile ? 28 : 45;
    const cols = isMobile ? 56 : 90;
    ...
    const animate = (timestamp) => {
      ...
      // Continuous rotation
      rotY += isMobile ? 0.0024 : 0.0035; // slower spin on mobile
